...
export async function GET() {
  ...
  const roleHistory: Record<string, Array<{ title: string; detail: string; timestamp?: string }>> = {
    ...
    owner: [
      { title: "Restaurant activity", detail: "Restaurant metrics refreshed", timestamp: new Date().toISOString() },
    ],
    rider: [
      { title: "Delivery activity", detail: "Dispatch record synchronized", timestamp: new Date().toISOString() },
    ],
    customer: [
      { title: "Order activity", detail: "Recent cart and order history refreshed", timestamp: new Date().toISOString() },
    ],
  };
