...
type Profit = {
  totals: { product_sales: string; platform_profit: string };
  restaurants: {
    id: number;
    name: string;
    order_count: string;
...
export default function AdminDashboard() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
                ...
                {restaurants.map((restaurant) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      ...
                      <span className="mt-1 block text-xs text-muted-foreground">
                        {restaurant.owner_phone || "No phone"}
                      </span>
                    </td>
        ...
        <section>
          ...
          <div className="border border-border bg-card">
            {riders.map((rider) => (
              <div
                key={rider.id}
                className="flex justify-between border-b border-border px-4 py-4 text-sm last:border-0"
              >
      ...
      {profit && (
        <section className="mt-10">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {profit.restaurants.map((restaurant) => (
              <div
                key={restaurant.id}
              ...
              >
                ...
                <div className="text-right">
                  <strong>{money(restaurant.total_sales)}</strong>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {money(restaurant.admin_profit)} fee
                  </p>
                </div>
              </div>
            ))}
      ...
      {selectedRestaurant && report && (
        <section className="mt-10 border-t border-border pt-8">
          <div className="flex items-center justify-between">
            <h2 className="font-serif text-2xl font-bold">
              {selectedRestaurant.name} detail
            </h2>
            <button
              onClick={() => {
          ...
          <div className="mt-4 grid gap-8 lg:grid-cols-2">
            <div>
              <h3 className="mb-3 font-semibold">Product sales</h3>
              {report.products.map((product) => (
                ...
                >
                  ...
                  <strong>{money(product.total_revenue)}</strong>
                </div>
              ))}
            </div>
            <div>
