...
type AdminOrderDetail = AdminOrder & { customer_phone: string | null; restaurant_address: string; transaction_id: string | null; rider_id: number | null; rider_phone: string | null };
...
export default function AdminDashboard() {
  ...
  async function updateOrder(orderId: number, changes: { orderStatus?: string; paymentStatus?: string; riderId?: number | null }) {
    try {
      const response = await fetch(`/api/admin/orders/${orderId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(changes) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Could not update order");
      await openOrder(orderId);
      if (payload.order) setOrders((current) => current.map((order) => order.id === orderId ? { ...order, order_status: payload.order.order_status } : order));
    } catch (updateError) {
  ...
  async function openRestaurant(restaurant: Restaurant) {
    ...
    const response = await fetch(
      `/api/admin/restaurants/${restaurant.id}?range=${range}`,
    );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
      <div className="mb-8 flex flex-col justify-between gap-4 border-b border-border pb-7 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Restaurants, riders, sales, and platform earnings in one place.
          </p>
        </div>
        <select
          value={range}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric
          label="Product sales"
          value={money(profit?.totals.product_sales ?? 0)}
        />
        <Metric
          label="Platform profit"
          value={money(profit?.totals.platform_profit ?? 0)}
        />
      </div>
      ...
      <section className="mt-10">
        ...
        {selectedOrder && <div className="mt-4 border border-border bg-card p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="font-serif text-xl font-bold">Order #{selectedOrder.id}</h3><p className="mt-1 text-sm text-muted-foreground">{selectedOrder.customer_name} · {selectedOrder.customer_email} · {selectedOrder.restaurant_name}</p></div><button onClick={() => setSelectedOrder(null)} className="text-sm text-muted-foreground hover:text-foreground">Close</button></div><div className="mt-4 grid gap-3 sm:grid-cols-4"><Metric label="Total" value={money(selectedOrder.total_amount)} /><label className="border border-border bg-background p-3"><span className="block text-[11px] uppercase tracking-wide text-muted-foreground">Order status</span><select value={selectedOrder.order_status} onChange={(event) => void updateOrder(selectedOrder.id, { orderStatus: event.target.value })} className="mt-2 w-full bg-transparent text-sm font-semibold"><option value="pending">Pending</option><option value="confirmed">Confirmed</option><option value="preparing">Preparing</option><option value="ready">Ready</option><option value="out_for_delivery">Out for delivery</option><option value="delivered">Delivered</option><option value="cancelled">Cancelled</option></select></label><label className="border border-border bg-background p-3"><span className="block text-[11px] uppercase tracking-wide text-muted-foreground">Payment</span><select value={selectedOrder.payment_status ?? ""} onChange={(event) => void updateOrder(selectedOrder.id, { paymentStatus: event.target.value })} className="mt-2 w-full bg-transparent text-sm font-semibold"><option value="pending">Pending</option><option value="completed">Completed</option><option value="failed">Failed</option><option value="refunded">Refunded</option></select></label><Metric label="Delivery" value={selectedOrder.delivery_status || "Unknown"} /></div><div className="mt-5 grid gap-6 lg:grid-cols-2"><div><h4 className="mb-2 font-semibold">Items</h4>{selectedOrderItems.map((item) => <div key={item.id} className="flex justify-between border-b border-border py-2 text-sm"><span>{item.item_name} × {item.quantity}</span><span>{money(item.subtotal)}</span></div>)}</div><div className="text-sm"><h4 className="mb-2 font-semibold">Payment and delivery</h4><p>Method: {selectedOrder.payment_method || "Not recorded"}</p><p>Transaction: {selectedOrder.transaction_id || "Not recorded"}</p><label className="mt-3 block"><span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Assigned rider</span><select value={selectedOrder.rider_id ?? ""} onChange={(event) => void updateOrder(selectedOrder.id, { riderId: event.target.value ? Number(event.target.value) : null })} className="w-full border border-border bg-background px-3 py-2 text-sm"><option value="">Unassigned</option>{riders.map((rider) => <option key={rider.id} value={rider.id}>{rider.name} · {rider.status}</option>)}</select></label><p className="mt-2">Phone: {selectedOrder.rider_phone || "Not available"}</p></div></div></div>}
      </section>
